...
class Nodo extends EventEmitter {
  ...
  constructor({ etiqueta, rpcPort = 50052, ofreceGB, modelo = MODELO, discovery, inferir: inf }) {
    super()
  ...
  start() {
    this.disco.on('listening', (info) => this.emit('listening', info))
  ...
  preguntar(texto, opciones = {}) {
    return this._inferir(this.plan(), { modelo: this.modelo, prompt: texto, ...opciones })
  }

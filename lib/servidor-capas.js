...
// ggml-rpc-server sale con codigo 0 cuando no puede tomar el puerto, asi que el codigo de
// salida no alcanza para distinguirlo. Esta linea si.
const OCUPADO = 'Failed to create server socket'
...
function servirCapas({
  ...
  try {
    ...
    const fallo = new Error(`could not start the layer server: ${detalle}`)
    fallo.code = err.code
    setTimeout(() => {
  ...
  const registrar = (d) => {
    ...
    // Si el puerto ya lo tiene otro proceso, nuestra sonda igual lo encuentra abierto y
    // creeria que arranco bien. Hay que decirlo antes de que eso pase.
    if (texto.includes(OCUPADO)) {
      fallar(new Error(`port ${puerto} is held by another process (an old rpc-server?)`))
    }
  }
  ...
  proc.on('exit', (code) => {
    const detalle = listo ? 'died' : 'never started'
    fallar(new Error(`the layer server ${detalle} (code ${code})`))
  })
  ...
  const golpear = () => {
    ...
    zocalo.on('error', () => {
      ...
      if (ahora() - desde > PACIENCIA) {
        fallar(new Error(`the layer server never opened port ${puerto}`))
        return
      }

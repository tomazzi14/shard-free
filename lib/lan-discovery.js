...
const { dgram, os, EventEmitter } = require('./runtime.js')

const PUERTO = 41234
const INTERVALO_BARRIDO = 5000 // ms entre barridos
const TANDA = 256 // ips por tanda, para no golpear la red de una
const PAUSA_TANDA = 40 // ms entre tandas
// Un peer se da por perdido a los 12s: dos barridos y medio de margen. Con 30s la pantalla
// se quedaba medio minuto en ambar despues de cerrar una maquina, que en cámara es eterno.
const TTL_PEER = 12000
const INTERVALO_LIMPIEZA = 2000 // cada cuanto revisamos quien no da senales
...
function ipsDeSubred(address, netmask) {
  ...
  if (total > MAX_IPS) throw new Error(`subnet too large (${total} ips, cap ${MAX_IPS})`)
...
class LanDiscovery extends EventEmitter {
  ...
  start() {
    ...
    try {
      this.socket.bind(this.puerto, '0.0.0.0', () => {
        ...
        if (iface === null) {
          this._fallo(new Error('no IPv4 network interface'))
          return
        }
        this.emit('listening', { puerto: this.puerto, ...iface })
        this._barrer()
        this.intervalos.push(setInterval(() => this._barrer(), INTERVALO_BARRIDO))
        this.intervalos.push(setInterval(() => this._limpiar(), INTERVALO_LIMPIEZA))
      })
    } catch (err) {
      this.activo = false
      const detalle =
        err.code === 'EADDRINUSE'
          ? `port ${this.puerto} is already taken (another instance running?)`
          : err.message
      this._fallo(new Error(`could not listen: ${detalle}`))
    }
  }
  ...
  _onMensaje(msg, rinfo) {
    ...
    try {
      ...
      this._fallo(new Error(`unreadable message from ${rinfo.address}: ${err.message}`))
      return
    }
  ...
  _registrar(sobre, address) {
    ...
    // La ficha de un peer cambia en vivo: arranca ofreciendo 0GB y pasa a ofrecer memoria
    // recien cuando su servidor de capas esta listo. Sin avisar del cambio, el que lo
    // descubrio primero se queda con la ficha vieja y nunca lo suma al reparto.
    if (JSON.stringify(conocido.ficha) !== JSON.stringify(peer.ficha)) {

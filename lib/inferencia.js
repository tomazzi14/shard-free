...
const COLA = 2000 // cuanto guardamos de cada salida para poder explicar un fallo
...
function construirArgs(plan, { modelo, prompt, ctx = 1024, nPredict = 64, fijarSplit = false }) {
  ...
  // --simple-io: sin esto llama-cli usa su interfaz de chat, que escribe directo a la
  // terminal y se saltea el pipe que le damos. Corriendo la app en una terminal, la
  // respuesta aparecia ahi y a nosotros no nos llegaba nada. Su propia ayuda lo dice:
  // "basic IO for better compatibility in subprocesses".
  // -st: una sola respuesta y termina. Sin esto abre un chat interactivo.
  args.push('--simple-io', '-st', '--no-warmup', '-p', prompt)
  return args
}
...
function inferir(plan, opciones) {
  ...
  if (!plan.completo) {
    const err = new Error(
      `model incomplete: layers ${plan.faltan.desde}-${plan.faltan.hasta} are missing`
    )
  ...
  try {
    ...
    const detalle = err.code === 'ENOENT' ? `${binario} is not there` : err.message
    setTimeout(() => emisor.emit('error', new Error(`could not start the model: ${detalle}`)), 0)
    return emisor
  }
  ...
  proc.stderr.on('data', (d) => {
    ...
    quejas = (quejas + texto).slice(-COLA)
  ...
  proc.on('exit', (code) => {
    if (code !== 0) {
      const err = new Error(`llama-cli exited with code ${code}: ${ultimaQueja(quejas)}`)
    ...
    if (!contesto) {
      ...
      const detalle = ultimaQueja(quejas) || ultimaQueja(cola) || 'it wrote nothing'
      const err = new Error(`the model returned nothing: ${detalle}`)
      err.crudo = cola
      emisor.emit('error', err)
      return
    }
